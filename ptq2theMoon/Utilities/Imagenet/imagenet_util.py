...
from ..Executor.myHook import MyHook
import pandas as pd
import onnxruntime
import torch
...
from ppq.executor.torch import TorchMetaDataTracingHook
from ppq.executor.torch import TorchExecutor
from ppq.IR import BaseGraph
from torch.utils.data.dataloader import DataLoader
from torch.utils.data.dataset import Subset
from tqdm import tqdm
from ..myDataloader import TinyImageNet
...
def _evaluate_any_module_with_imagenet(
    ...
    if verbose:
        print(' * Prec@1 {top1:.3f} Prec@5 {top5:.3f} Inference_time {time:.3f} avg_loss {avg_loss:.4f}'.format(
            top1=sum(recorder['top1_accuracy']) / len(recorder['top1_accuracy']),
            top5=sum(recorder['top5_accuracy']) / len(recorder['top5_accuracy']),
            time=(sum(recorder['batch_time']) / len(recorder['batch_time'])) / batchsize,
            avg_loss=(sum(recorder['loss']) / len(recorder['loss'])) / batchsize))
